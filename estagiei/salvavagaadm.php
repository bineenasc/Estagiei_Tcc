<?php session_start();
   ...
   '$Valor', '$TempoEstudo', '$PeriodoTrabalho', '$Site')";
    ...
    if(mysqli_query($con, $sql)){ 	//Aqui o mysqli_query está fazendo uma inserção no banco de dados
        header("location: vagas_cadastradas.php"); 	//Caso a inserção seja feita com sucesso, a página será redirecionada ao index.html
        mysqli_close($con); 	//Fechando a conexão, é muito importante que ela seja fechada!!!!
    }
    else{
        echo "<script>alert('Erro ao incluir dados!');window.location='vagas_cadastradas.php'</script>";
        mysqli_close($con);
    }
